...
def GetModel(convFilters:np.array,denseNeurons:np.array,convDropout:np.array, denseDropout:np.array, SpatialDropout=False, LossFunction=snorm_MSE):
    """
    Function to create a 3D CNN model with two inputs: 3D data and energies.

    - convFilters: np.array of integers defining the number of filters in each convolutional layer.
    ...
    # 3D CNN Path
    x = input_3d
    for nFeatures, drops in zip(convFilters, convDropout):
        if drops == 0:
            ...
            if SpatialDropout:
                x = layers.Conv3D(nFeatures, kernel_size=(3, 3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01))(x)
                x = layers.SpatialDropout3D(drops)(x)
                x = layers.MaxPooling3D(pool_size=(2, 2, 2))(x)
            else:
                x = layers.Conv3D(nFeatures, kernel_size=(3, 3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01))(x)
                x = layers.Dropout(drops)(x)
                x = layers.MaxPooling3D(pool_size=(2, 2, 2))(x)
    ...
    x = combined
    for nNeurons, denseDrops in zip(denseNeurons, denseDropout):
        x = layers.Dense(nNeurons, activation='relu')(x)
        x = layers.Dropout(denseDrops)(x)
...
hyperparameters = [ [[0,0,0],[0.2,0,0],False], [[0,0,0],[0,0.2,0],False], [[0,0,0],[0,0,0.2],False], 
                   [[0,0,0],[0.1,0.1,0.1],False], [[0,0,0],[0,0,0.3],False] ,[[0,0,0.1],[0,0,0],False], [[0,0,0.2],[0,0,0],False], [[0.1,0.1,0.1],[0,0,0],True]]
# DEFINE THE MODEL
Models = []
...
names=['Chonky_dense_Dropout(1,0.2)','Chonky_dense_Dropout(2,0.2)','Chonky_dense_Dropout(3,0.2)','Chonky_dense_Dropout(1,2,3,0.1,0.1,0.1)','Chonky_dense_Dropout(3,0.3)','Chonky_convo_Dropout(3,0.1)', 'Chonky_convo_Dropout(3,0.2)', 'Chonky_convo_SDropout(1,2,3,0.1,0.1,0.1)']
