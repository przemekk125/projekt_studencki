...
# DEFINE THE MODEL
conv_regularizers = [regularizers.l2(1e-4), 
                     regularizers.l2(1e-4), 
                     regularizers.l2(1e-3), 
                     regularizers.l2(1e-4), 
                     regularizers.l2(1e-4)]
dense_regularizers = [regularizers.L1L2(l1=1e-5, l2=1e-3), 
                      regularizers.L1L2(l1=1e-3, l2=1e-2), 
                      regularizers.L1L2(l1=1e-5, l2=1e-3),
                      regularizers.L2(1e-3), 
                      regularizers.L1(1e-4)]

models = []
for reg_conv,reg_dense in zip(conv_regularizers, dense_regularizers):  
    models.append(GetModel(convFilters=[32,32,16],denseNeurons=[64,32,16], 
                           reg_conv=reg_conv, reg_dense=reg_dense, LossFunction=snorm_MSE))
    
names = [f'model{i}' for i in range(1, 6)]
