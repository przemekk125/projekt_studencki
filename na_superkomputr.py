...
#losses=[Huber(delta=150),Huber(delta=170),Huber(delta=190),norm_MAE, snorm_MAE,norm_RMS,snorm_RMS, norm_Huber(d=150), norm_Huber(d=170),norm_Huber(d=190),snorm_Huber(d=150),snorm_Huber(d=170),snorm_Huber(d=190)]
#nazwa=['Huber(delta=150)','Huber(delta=170),Huber(delta=190)','norm_MAE', 'snorm_MAE','norm_RMS','snorm_RMS', 'norm_Huber(d=150)', 'norm_Huber(d=170)','norm_Huber(d=190)','snorm_Huber(d=150)','snorm_Huber(d=170)','snorm_Huber(d=190)']
losses=[norm_MAE, snorm_MAE,norm_RMS,snorm_RMS]
nazwa=['norm_MAE', 'snorm_MAE','norm_RMS','snorm_RMS']
ID=0
for loss in losses:
    metrics=[]
    model = Model(inputs=[input_3d], outputs=output)
    for E in Energies:
        metrics.append(ValLossForE(E=E, loss_func=loss))
    model.compile(optimizer='adam',
              loss=loss,
              metrics=metrics) 
    csv_logger = tf.keras.callbacks.CSVLogger(f"training_log{ID}.csv", append=True)
...
#    print("Wytrenowane! "+nazwa[ID])
    ...
    SaveModel(ME,MAE,normMAE, RMS,nazwa[ID]+'.csv',model )
    ID=ID+1
