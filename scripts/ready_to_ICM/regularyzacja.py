...
def GetModel(convFilters:np.array,denseNeurons:np.array,LossFunction=snorm_MSE):
    ...
    for nFeatures in convFilters:
        x = layers.Conv3D(nFeatures, kernel_size=(3, 3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(1e-4))(x)
    ...
    for nNeurons in denseNeurons:
        x = layers.Dense(nNeurons, activation='relu', kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-3))(x)
