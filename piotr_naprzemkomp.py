...
dataset = tf.data.Dataset.from_generator(
    ...
    output_signature=(
        ...
        tf.TensorSpec(shape=(1,), dtype=tf.float32,name = 'energy')),  # Label shape
        tf.TensorSpec(shape=(1,), dtype=tf.int16,name = 'y')  # Sum energy shape
    )
)
...
model.save('piotr_no_metrics_maybefaster.h5')